...
library(tidyverse); library(tidygraph); library(ggraph); library(magrittr)
url <- "https://pure.abdn.ac.uk/ws/portalfiles/portal/126193001/Odyssey.txt"

df <- read.delim(url,sep = "\ ",encoding = "latin1") %>% select(from = var1,to = var2)
...
ntwk %<>%
  ...
  # Make char subset
  mutate(keyplayer_name = if_else(keyplayer,name,NA_character_))
  
```

Non-statsy definitions for node measures:

- *Betweenness*: Betweenness is (roughly) defined by the number of geodesics (shortest paths) going through a vertex or an edge. If St. Sebastian was a node, he would have a high betweenness score when relationships were depicted by arrow shots.
...
ntwk %<>%
  activate(nodes) %>% 
  mutate(
    grp_infomap = group_infomap(),
    grp_between = group_edge_betweenness(),
    grp_louvain = group_louvain(),
    grp_anneal  = group_spinglass()
  )
...
ntwk_mds  %>%
  ...
  geom_node_text(aes(label = keyplayer_name), repel = T) +
...
ntwk %>%
  ggraph() + 
  geom_node_voronoi(
    aes(fill = as.character(grp_louvain)), 
    max.radius = .75, colour = 'white'
  ) + 
  geom_edge_link(alpha = 0.25) + 
  geom_node_point(
    aes(colour = as.character(grp_anneal)),
    alpha = 0.5
  ) +
...
ntwk %>%
  ...
  geom_node_range(aes(colour = as.character(grp_anneal)), alpha = 0.3) + 
