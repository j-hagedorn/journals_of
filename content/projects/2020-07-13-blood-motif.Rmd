...
Recently, re-reading the first volume of Mircea Eliade's *A History of Religious Ideas* (the one which covers the period from the Stone Age to the Eleusinian Mysteries), I found myself missing the author: his encyclopedic knowledge and deftness in analogy and synthesis.  Despite the breadth of his existing work, I wanted to have him beside me to tell me stories and find connections within the vast web of human myths and stories.

Eliade himself could have woven together an array of myths pointing to such a 'resurrection', and the attendant failures.  Eventually, I settled (as we seem to do more and more) for that inert facsimile of the never-resting mind: the encyclopedia, perhaps with hyperlinks to stand in for analogies.  In this search I happened upon Stith Thompson's massive labour of love, the [Motif-Index of Folk-Literature](https://en.wikisource.org/wiki/Motif-Index_of_Folk-Literature/Volume_1) which contains `r nrow(motifs)` distinct motifs: the building blocks of "Folktales, Ballads, Myths, Fables, Mediaeval Romances, Exempla, Fabliaux, Jest-Books and Local Legends."  While the index has its limitations for research purposes,^[See d’Huy, J., Le Quellec, J. L., Berezkin, Y., Lajoye, P., & Uther, H. J. (2017). Studying folktale diffusion needs unbiased dataset. Proceedings of the National Academy of Sciences, 114(41), E8555-E8555.] it serves for my initial goal: the ability to give a broad, impressionistic view of a range of related themes throughout myths.

It turns out that can do [some web-scraping and tidying up](https://github.com/j-hagedorn/mythos) to arrive at a working dataset for these motifs. For methods, I'm starting out pretty basic.  After a few forays applying topic modeling, with results which strayed too far from the types of nuanced categories I had hoped for, I settled on simple tagging of keywords using regular expressions: a bit more work, but more useful output.  Even this is an admittedly messy approach,^[In order to apply a single theme to a given motif which may have matched multiple search criteria, I used a `case when` statement which explicitly chooses an order of importance and individual motifs to the first group for which it meets criteria.  A refined approach might be to use the gamma score from topic model output, if those could be made to generate useful groupings.  Then a given motif could be associated more or less strongly with multiple themes.] as the grouping of some motifs shows, but I have resisted re-classifying the initial sketch made, which aims at a broader synopsis and impression rather than an unassailable classification system.
...
And what is 'in our blood' is not merely hemoglobin or lipoproteins, but an entire history of resonances, of stories and ideas about ourselves which color the substance coursing through our veins.  

Searching the Thompson Motif Index for 'blood' or 'bleed', we find `r nrow(blood)` entries.  By reading through these and flagging keywords which correspond to identified themes, I ended up with a reduced set of `r nrow(blood_theme)` entries categorized by theme.^[Note that a substantial number of motifs (n = `r nrow(blood) - nrow(blood_theme)`) including the words 'blood' or 'bleed' were not grouped using the search criteria applied.  Most of these have to do with tales, such as those involving ogres visiting violence on their victims, etc.]  Culling these `r length(unique(blood_theme$theme))` themes into `r length(unique(blood_theme$group))` broader groups yielded the categories used in the discussion below.
...
Some of these have distinct, smaller clusters of themes congealed within them, so it is worth dwelling on each in greater detail, as well as providing illustrations of the variety of motifs within each group.  The overall structure of these groups, subgroups, and motifs is shown below as a circular network:
...
Each set of branches from the trunk of this network is briefly discussed in a section below, to give a sense of the diversity of myths and stories represented.

## Cosmic Foundation

The first, and largest, set of themes which seemed to cluster together are those about the essence of blood.  Vital and numinous, blood seethes with the creative power of life itself.  It is related to, and in some instances constitutes, the substance of the soul, which has previously been defined quite differently than our current usage.  For the Greeks, the *soul* was not merely a wisp but the patterning power which governed the unfolding and development of the body. Blood is shown in this way in motifs which describe its nature as soul/spirit or which link it to the basic elemental forces of the world or to the initial creation of the cosmos (i.e. cosmogonic myths).
...
When blood comes from certain inanimate objects it reveals the life or more substantial reality of those objects, thereby associating them with reality or truth.  In Irish myths, a shrine bleeds to identify its living connection with the reality,^[`r cite("F991.4")`] and a desecrated altar also bleeds to communicate the continued power of reality beyond that impure act.^[`r cite("F991.4.1")`]
...
When not explicitly tied to the reasons why people were drinking blood (e.g. to transform themselves, or make a pact, etc.), I classified motifs which treated the drinking of blood in a separate category.

While the reason for drinking blood may not be clear in these motifs, the act itself is of interest.  Other means of applying blood, such as baths, are present and mentioned above.  The ingestion of another creatures blood, the taking of external life into one's own body through the mouth, the same route by which the body accepts all other nourishment, is so central to myth and to Christian practice that it is worth dwelling upon.
