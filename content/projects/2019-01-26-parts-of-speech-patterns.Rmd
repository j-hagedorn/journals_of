...
Written language is everywhere, making it especially poignant to acknowledge what we' ha've lost in branching away from oral culture.  Yet it has much to love, and one of the things that written language can give us, with its static wings, is a preserved body to stare into: beyond the expiration of air and the inter-personal awkwardness of staring.

Just as Ramon y Cahal segmented and sketch out neuronal cells, making drawings which were both accurate and lovely, we might sketch out the patterns of utterance: not to be useful or prove some theory, but merely to observe the unintentional patterns that a voice accumulates across the duration of its expeditions and mappings.

We might hold out hope that words, even set down, continue to hold the sparks of personhood. As Coleridge proposes: "...if words are not THINGS, they are LIVING POWERS, by which the things of most importance to mankind are actuated, combined, and humanized."^[Coleridge, S.T. *Aids to Reflection*, ed. John Beer (1993), p. 10] 
...
Here are all of the phrases from Milton's *Paradise Lost*.  Starting from the blank hole at the center and proceeding outward, one sees what percent of phrases follow which sequence of parts of speech:
...
What if we look at a different poet's work?  Will it look obviously different, or does the form of the language itself (the ligaments of its connections) overwhelm any patterns peculiar to a single person?^[Is this an instance of the models we use (e.g. language) speaking louder than our individual voices, or a story about the inadequacy of models to describe the utterance of an individual riding its rules?  Are those stories the same one, with different narrators? ]  Here are the collected poems of Keats:
...
To get an inkling of whether there are patterns that are peculiar to one poet more than another, I did a little sequential association rule mining.^[This analysis uses the association rule mining package referenced in Hahsler M, et al. (2005a). *"arules – A Computational Environment for Mining Association Rules and Frequent Item Sets."* Journal of Statistical Software, 14(15), 1–25.]  With this approach, pattern sequences are scored using the following characteristics for each body of work (Milton & Keats):
...
In the plot below, each dot stands for a rule/pattern which is found in both poets' work, using the `lift` measure described just above. The colors are based on differnce, so that yellow means "this pattern is pretty similar between both" and purple means "this pattern differs most between them".
...
```{r rule_scatter,fig.showtext=TRUE,fig.retina=1}
showtext_auto()

seq_rules_df %>%
  filter(complete.cases(.)) %>%
  mutate(
    difference = confidence_milton - confidence_keats,
    some_labels = if_else(
  ...
  ggplot(
    aes(
      x = confidence_milton, y = confidence_keats, 
  ...
  theme_minimal() +
  theme(text=element_text(size = 8,family="merriweather"))

# Open device to print with font
# windows(); print(p); dev.off()
```
...
A footprint in the dust is not equal to a foot. And yet the traces that we leave hold some remnant of us, beyond our intentions and our desire to trace any particular legacy.  And if the momentary accident of a single phrase can delight with a striking analogy, perhaps the accidental/unintentional patterns of phrasing over the course of one’s life may also spark some slight delight.
